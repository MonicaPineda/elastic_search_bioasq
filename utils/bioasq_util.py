...
def search_docs_with_score(question, index_name, num_docs,remove_tags=False):
    doc_list = []
    try:
        query = {
            "from" : 0, 
        ...
        res = es.search(index=index_name, body=query, request_timeout=40)
        for doc in res['hits']['hits']:
            doc_id = doc['_id']
            doc_title = doc['_source']['title']
            doc_abstract = doc['_source']['abstract']
            ...
            if remove_tags:
                doc_abstract = re.sub("<.*?>", "", doc_abstract)
            doc_list += [('http://www.ncbi.nlm.nih.gov/pubmed/'+doc_id, doc_title, doc_abstract, doc_mesh, score)]
...
def search_docs_with_score_date(question, index_name, num_docs,remove_tags=False):
    doc_list = []
    try:
        query = {
            "from" : 0, 
        ...
        for doc in res['hits']['hits']:
            ...
            doc_abstract = doc['_source']['abstract']
            doc_mesh = doc['_source']['mesh']
            year = doc['_source']['created_date'][0:4]
            score= doc['_score']
            if remove_tags:
                doc_abstract = re.sub("<.*?>", "", doc_abstract)
...
def search_docs_mesh(question, index_name, num_docs, remove_tags=False):
    ...
    try:
        ...
        for doc in res['hits']['hits']:
            ...
            if remove_tags:
                doc_abstract = re.sub("<.*?>", "", doc_abstract)
            doc_list += [('http://www.ncbi.nlm.nih.gov/pubmed/'+doc_id, doc_title, doc_abstract, doc_mesh)]
